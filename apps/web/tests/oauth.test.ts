...
describe("Google OAuth routes", () => {
  ...
  describe("GET /api/auth/google/callback", () => {
    it("exchanges the code, upserts the user, sets session cookie, and 302s /dashboard", async () => {
      ...
      server.use(
        http.post("https://oauth2.googleapis.com/token", async ({ request }) => {
          tokenCalls.push(await request.text());
          return HttpResponse.json(tokenJson());
        }),
      );

      // Mint a real nonce via the start endpoint so the state shape matches.
      const startRes = await startGet(new NextRequest("http://localhost:3000/api/auth/google/start"));
    ...
    it("redirects with code_exchange_failed when Google returns 400 on /token", async () => {
      ...
      expect(res.status).toBe(302);
      expect(res.headers.get("location")).toBe("http://localhost:3000/?oauthError=code_exchange_failed");
    });
    ...
    it("preserves the prior refresh token when Google omits it on re-consent", async () => {
      ...
      await pool.query(
        ...
        [GOOGLE_EMAIL, GOOGLE_SUB, "1//PRIOR-REFRESH", "old-access", new Date().toISOString(), "Demo", null],
      );
      ...
      const { rows } = await pool.query<{ google_refresh_token: string; google_access_token: string }>(
  ...
  describe("GET /api/auth/me", () => {
    ...
    it("returns the user shape when a valid session cookie is present", async () => {
      ...
      const body = (await res.json()) as { user: { id: string; email: string; displayName: string | null; pictureUrl: string | null } };
  ...
  describe("POST /api/auth/google/disconnect", () => {
    it("revokes at Google, nulls the Google columns, and clears the cookie", async () => {
      ...
      await pool.query(
        ...
        [userId, "dc@test.com", "sub-dc", "1//LIVE", "ya29.LIVE", new Date().toISOString(), "DC", null],
      );
      const token = await signSession({
        sub: userId,
        email: "dc@test.com",
      ...
      const { rows } = await pool.query<{ google_sub: string | null; google_refresh_token: string | null }>(
        "SELECT google_sub, google_refresh_token FROM users WHERE id=$1",
        [userId],
      );
