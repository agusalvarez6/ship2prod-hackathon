...
import { applyMigrations, openDb, openRedis, primeTestEnv, truncateUsers } from './helpers.js'
...
process.env.NOTION_OAUTH_REDIRECT_URI = 'http://localhost:3000/api/auth/notion/callback'
...
const { POST: disconnectPost } = await import('../src/app/api/auth/notion/disconnect/route.js')
...
const { TransientError, PermanentError, UserInputError } = await import('../src/lib/errors.js')
...
async function seedUser(pool: Pool, id: string, email: string): Promise<void> {
  await pool.query(`INSERT INTO users (id, email, google_sub) VALUES ($1, $2, $3)`, [
    id,
    email,
    `sub-${id}`,
  ])
...
describe('Notion OAuth routes', () => {
  ...
  describe('exchangeNotionCode', () => {
    it('POSTs JSON with HTTP Basic auth and returns the parsed token response', async () => {
      ...
      const expectedBasic = Buffer.from('test-notion-client-id:test-notion-client-secret').toString(
        'base64',
      )
  ...
  describe('GET /api/auth/notion/callback', () => {
    ...
    it('exchanges code, stores token on users.notion_token, redirects /dashboard?notionConnected=1', async () => {
      ...
      expect(res.status).toBe(302)
      expect(res.headers.get('location')).toBe('http://localhost:3000/dashboard?notionConnected=1')
    ...
    it('redirects with code_exchange_failed when Notion rejects the code', async () => {
      ...
      const res = await callbackGet(
        new NextRequest(`http://localhost:3000/api/auth/notion/callback?code=c&state=${nonce}`, {
          headers: { cookie },
        }),
      )
      expect(res.status).toBe(302)
    ...
    it("forwards Notion's error param when the user cancels", async () => {
      const res = await callbackGet(
        new NextRequest('http://localhost:3000/api/auth/notion/callback?error=access_denied'),
      )
      expect(res.status).toBe(302)
      expect(res.headers.get('location')).toBe(
  ...
  describe('POST /api/auth/notion/disconnect', () => {
    ...
    it('nulls notion_token and returns 204', async () => {
      await pool.query(`UPDATE users SET notion_token = $1 WHERE id = $2`, ['secret_LIVE', USER_ID])
