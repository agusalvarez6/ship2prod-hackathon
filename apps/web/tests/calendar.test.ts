...
function calendarItem(
  overrides: Partial<{ id: string; summary: string; start: string; end: string }> = {},
) {
  return {
  ...
  it('refreshes an expired access token before calling Google Calendar', async () => {
    ...
    server.use(
      http.post('https://oauth2.googleapis.com/token', async ({ request }) => {
        refreshBody = await request.clone().text()
        return HttpResponse.json({
  ...
  it('inserts the event on Google with summary + start/end and returns the trimmed event', async () => {
    ...
    server.use(
      http.post(GOOGLE_EVENTS_URL, async ({ request }) => {
        receivedBody = (await request.clone().json()) as typeof receivedBody
        return HttpResponse.json({
          id: 'evt-created',
    ...
    const body = (await res.json()) as {
      event: { id: string; title: string; htmlLink: string | null }
    }
